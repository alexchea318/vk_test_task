...
❗ Сайт протестирован в браузерах Firefox, Edge, Chrome, Safari(iphone), Chrome Mobile(Android). Используются современные CSS свойства, работа в старых браузерах может быть некорректной.
...
5. Подсветка хештегов, ссылок, почт и упоминай реализована с помощью **регулярных выражений** динамически в режиме реального времени
6. Присутствует **проверка на корректность**: например, если из почты удалить собаку, подсветка исчезнет.
...
10. Подсвечивается только хештег или упоминание, а не все слово. Например, если ввести 1@sasha - подсветится только @sasha.
...
13. Подсветка остается в сообщениях
